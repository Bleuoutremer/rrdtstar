...
Usage:
  main.py (rrdt|rrdtinformed|rrt|birrt|informedrrt|prm|particle|likelihood|nearby|mouse) <MAP>
          [options] [-v|-vv|-vvv]
  main.py (rrdt|rrdtinformed|rrt|birrt|informedrrt|prm|particle|likelihood|nearby|mouse) <MAP>
...
def main():
    ...
    elif args['rrdtinformed']:
        from planners.rrdtinformed import RRdTInformedSampler, RRdTInformedPlanner
        sampler = RRdTInformedSampler(
            restart_when_merge=not args['--no-restart-when-merge'])
        planner_type = RRdTInformedPlanner
